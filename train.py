    ...
    for epoch in range(args.start_epoch, psnr_epochs):
        ...
        for i, (input, target) in progress_bar:
            ...
            if (total_iter + 1) % 5000 == 0:
                vutils.save_image(lr, os.path.join(output_lr_dir, f"RFDBNet_PSNR_{total_iter + 1}.bmp"))
                vutils.save_image(hr, os.path.join(output_hr_dir, f"RFDBNet_PSNR_{total_iter + 1}.bmp"))
                vutils.save_image(sr, os.path.join(output_sr_dir, f"RFDBNet_PSNR_{total_iter + 1}.bmp"))
...
for epoch in range(args.start_epoch, epochs):
    ...
    for i, (input, target) in progress_bar:
        ...
        # The image is saved every 5000 iterations.
        if (total_iter + 1) % 5000 == 0:
            vutils.save_image(lr, os.path.join(output_lr_dir, f"RFB_ESRGAN_{total_iter + 1}.bmp"))
            vutils.save_image(hr, os.path.join(output_hr_dir, f"RFB_ESRGAN_{total_iter + 1}.bmp"))
            vutils.save_image(sr, os.path.join(output_sr_dir, f"RFB_ESRGAN_{total_iter + 1}.bmp"))
